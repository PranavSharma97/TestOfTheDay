...
public class PreviousTestsWrongQuestions extends ActionBarActivity {

	RadioGroup answerGroup;
	RadioButton option1, option2, option3, option4;
	TextView difficultyLevel;
	TextView question,wrongTv;
	String strResult;
	String explanation;
	...
	static int tempCurrentIndex=0;
	static int tempTestNumber;
	static boolean allTestOver = false;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_previoustestswrong_questions);
...
//		questiondAndOptions = Questions.questionslist;
		...
		wrongTv = (TextView)findViewById(R.id.wrongQuestions);
		wrongTv.setText("INCORRECT QUESTIONS");
...
//		Toast.makeText(this,
//				 "Your Answer " + tempRadioButton.toString(), Toast.LENGTH_SHORT).show();
...
//				submitButton.setText("Try Again");
	...
	void showDialog() {
		...
		{
			if(tempIndex==PreviousTestsActivity.wrongIndex-1)
				showTestEndDialog();
			else 
				return;
		}
		else if(tempIndex==PreviousTestsActivity.wrongIndex-1)
				{showTestEndDialog();
				}
		else
		{
...
//		Toast.makeText(getApplicationContext(), "current : " + PreviousTestsActivity.currentIndex + " wrong : " + PreviousTestsActivity.wrongIndex, Toast.LENGTH_LONG).show();
		isReviewed=true;
		tempCurrentIndex = PreviousTestsActivity.currentIndex;
		tempTestNumber = PreviousTestsActivity.testNumber;
		AlertDialog dialog;
		dialog = new AlertDialog.Builder(this).create();
		...
		dialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Next Paper",
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						if(tempCurrentIndex-1 == PreviousTestsActivity.finalIndex)
						{
							allTestOver = true;
							Toast.makeText(getApplicationContext(), "All Test  Over" , Toast.LENGTH_LONG).show();
							Intent intent;
						...
						Intent intentHomeActivity = new Intent(
								getApplicationContext(), PreviousTestsActivity.class);
						startActivity(intentHomeActivity);
//						PreviousTestsActivity.goToResetAllData();
						finish();
						}
					}
				});
		dialog.setButton(DialogInterface.BUTTON_POSITIVE, "Review Right Questions",
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {

						Intent intentHomeActivity = new Intent(
								getApplicationContext(), PreviousTestsReviewPaper.class);
						startActivity(intentHomeActivity);
					}
				});
...
//				});
	...
	{
		questiondAndOptions = PreviousTestsActivity.wrongQuestions;
	}
